...
const char CompactHoughData::sSource[] = OCL_PROGRAM_SOURCE(
...
kernel void reduce_sum(read_only image2d_t image, int value, global int* p_block_sum)
{
...
kernel void prefix_sum_compact(read_only image2d_t image, int value, global int* p_blk_sum, global HoughData* p_out, int maxOutSize, global int* p_out_size)
{
    local int sum_blk;
...
CompactHoughData::CompactHoughData(cl::Context& ctxt, cl::CommandQueue& queue)
    :mContext(ctxt),
     mQueue(queue),
     mScanBlkSize(256),
     mReduceBlkSize(64),
...
CompactHoughData::~CompactHoughData()
{
}

void CompactHoughData::init(int warp_size)
{
...
void CompactHoughData::createIntBuffer(size_t buffSize)
{
    size_t memSize = 0;
...
size_t CompactHoughData::process(const cl::Image& inpImage, Ocl::DataBuffer<Ocl::HoughData>& out, int threshold, size_t& outCount)
{
    size_t width, height;
    ...
    mReduceKernel.setArg(1, threshold);
    ...
    mCompactScanKernel.setArg(1, threshold);
