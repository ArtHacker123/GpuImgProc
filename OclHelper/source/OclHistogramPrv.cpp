...
HistogramPrv::HistogramPrv(cl::Context& ctxt, cl::CommandQueue& queue)
    :mContext(ctxt),
     mQueue(queue)
{
    try
...
size_t HistogramPrv::computeTempHist(const cl::Image& image, size_t& count)
{
    size_t width, height;
    ...
    if (format.image_channel_order == CL_RGBA)
    {
        ...
        case CL_UNORM_INT8:
            ...
            mQueue.enqueueNDRangeKernel(mTempHistFloat, cl::NullRange, cl::NDRange(32 * wSizeX, height), cl::NDRange(32, 8), NULL, &event);
            event.wait();
        ...
        case CL_UNSIGNED_INT8:
            ...
            mQueue.enqueueNDRangeKernel(mTempHistUint8, cl::NullRange, cl::NDRange(32 * wSizeX, height), cl::NDRange(32, 8), NULL, &event);
            event.wait();
            time = (event.getProfilingInfo<CL_PROFILING_COMMAND_END>() - event.getProfilingInfo<CL_PROFILING_COMMAND_START>());
            break;
        }
...
size_t HistogramPrv::accumTempHist(size_t count, Ocl::DataBuffer<int>& rgbBins)
{
...
size_t HistogramPrv::compute(const cl::ImageGL& image, Ocl::DataBuffer<int>& rgbBins)
{
    size_t count = 0;
...
size_t HistogramPrv::compute(const cl::Image2D& image, Ocl::DataBuffer<int>& rgbBins)
{
    size_t count = 0;
    size_t time = computeTempHist(image, count);
    time += accumTempHist(count, rgbBins);
