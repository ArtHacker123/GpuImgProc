...
kernel void gauss(read_only image2d_t inpImg, write_only image2d_t outImg, global const float* pCoeffs, local float* shImgData)
{
    ...
    for (int j = 0; j < 5; j++)
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            idata += (pCoeffs[index++]*shImgData[offset]);
        }
    }
    write_imagef(outImg, (int2)(get_global_id(0), get_global_id(1)), idata);
}

kernel void non_max_edge_suppress(read_only image2d_t inpImg, write_only image2d_t outImg, local float* shImgData)
{
    loadImageData(inpImg, shImgData, 1, 1);
    barrier(CLK_LOCAL_MEM_FENCE);
    ...
    {
        if (idata <= shImgData[(xwidth*(q-1))+p+1] || idata <= shImgData[(xwidth*(q+1))+p-1])
        {
            idata = 0.0f;
        }
    }
    ...
    {
        if (idata <= shImgData[index-xwidth] || idata <= shImgData[index+xwidth])
        {
            idata = 0.0f;
        }
    }
    ...
    {
        if (idata <= shImgData[index - 1] || idata <= shImgData[index + 1])
        {
            idata = 0.0f;
        }
    }
