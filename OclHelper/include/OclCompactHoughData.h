...
class CompactHoughData
{
public:
    CompactHoughData(cl::Context& ctxt, cl::CommandQueue& queue);
    ~CompactHoughData();

public:
    size_t process(const cl::Image& inpImage, Ocl::DataBuffer<Ocl::HoughData>& out, int threshold, size_t& outCount);
